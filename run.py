...
from trains.singleTask.model import CADET
...
def _run(args, num_workers=4, is_tune=False, from_sena=False): 
    ...
    if args.is_training:
        ...
        model = []
        model_CADET = getattr(CADET, 'CADET')(args)
        ...
        model_CADET = model_CADET.cuda()

        model = [model_CADET]         
    else:
        print("testing phase for CADET")
        model = getattr(CADET, 'CADET')(args)
        model = model.cuda()
    ...
    #test
    if args.mode == 'test':
        model.load_state_dict(torch.load('./pt/CADET'+str(args.dataset_name)+'.pth'),strict=False) 
    ...
    #train
        ...
        # model[0].load_state_dict(torch.load('./pt/CADET'+str(args.dataset_name)+'.pth'))
